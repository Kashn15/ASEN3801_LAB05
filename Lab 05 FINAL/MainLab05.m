...
Problem2_2 = 0; 
Problem2_3 = 0; % Spiral
Problem3_1 = 0; % Short Period
Problem3_2 = 0; % Phugoid
...
% Simulation Time (For Problem 2.1-2.2)
t_f = 100; % [s]
...
if Problem3_1 == 1
    ...
    % ode45
    [t31, xdot_4] = ode45(@(t, x) AircraftEOMDoublet(t, x, AC_ctrl31, doublet_size, doublet_time, wind_inertial, aircraft_parameters), [0, t_f], AC_X31);
...
if Problem3_2 == 1
    ...
    AC_ctrl32 = [0.1079; 0; 0; 0.3182]; % u0 - Control input
    wind_inertial = [0;0;0]; % No wind

    odefun = @(T, x32) AircraftEOMDoublet(T, x32, AC_ctrl32, doublet_size, doublet_time, wind_inertial, aircraft_parameters);
    [t32, xdot_5] = ode45(odefun, [0, t_f_ph], AC_X32);
    ...
    for i = 1:length(t32) 
        ti = t32(i);
        if ti > 0 && ti <= doublet_time
            de_32(i) = AC_ctrl32(1) + doublet_size;
        elseif ti > doublet_time && ti <= 2*doublet_time
            de_32(i) = AC_ctrl32(1) - doublet_size;
        else
            de_32(i) = AC_ctrl32(1);
    ...
    % Other control surfaces
    da_32 = AC_ctrl32(2) * ones(size(t32)); % Ailerons
    dr_32 = AC_ctrl32(3) * ones(size(t32)); % Rudder
    dt_32 = AC_ctrl32(4) * ones(size(t32)); % Throttle
